...
while istop == 0
    ...
    [parcruzar,iparcruzar,resp_pop,elite,ielite] = selection(pop_bin_actual,pop_dec_actual,f); % Seleciona os indivíduos   % Seleciona os indíviduos a partir da aptidão
    
    if pcross > rand 
        [filhos] = crossover(parcruzar,nbits);  % Realiza o crossover a partir da probabilidade
        if pmut > rand
            [filhosm] = mutate(filhos,nbits); % Realiza mutação a partir da probabilidade
            filhos = filhosm;
        end
        [pop_bin_new] = evolve(filhos,pop_bin_actual,parcruzar,iparcruzar,resp_pop,elite,ielite,f,pop_dec_actual); % Evolui a espécie
